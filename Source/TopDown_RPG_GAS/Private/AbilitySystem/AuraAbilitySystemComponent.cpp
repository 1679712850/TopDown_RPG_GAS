...
void UAuraAbilitySystemComponent::AbilityInputTagPressed(const FGameplayTag& InputTag)
{
	...
	for (FGameplayAbilitySpec& AbilitySpec : GetActivatableAbilities())
	{
		if (AbilitySpec.GetDynamicSpecSourceTags().HasTagExact(InputTag))
		{
			...
			{
				for (auto Instance : AbilitySpec.GetAbilityInstances())
				{
					if (Instance)
					{
						const FPredictionKey PredKey = Instance->GetCurrentActivationInfo().GetActivationPredictionKey();
...
{
	const UAbilityInfoDataAsset* AbilityInfo = UAuraAbilitySystemBlueprintLibrary::GetAbilityInfo(GetAvatarActor());
...
{
	auto AbilityInfo = UAuraAbilitySystemBlueprintLibrary::GetAbilityInfo(GetAvatarActor());
